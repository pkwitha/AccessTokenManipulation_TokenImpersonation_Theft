...
BOOL setPrivilege(LPCTSTR priv) {
    ...
    LPCTSTR lpszPrivilege = TEXT("SeDebugPrivilege"); // name of privilege to enable/disable
    BOOL bEnablePrivilege = TRUE; // to enable or disable privilege

    TOKEN_PRIVILEGES tp; // used by local system to identify the privilege
    LUID luid{};
    if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES, &hToken)) bool res = FALSE;
